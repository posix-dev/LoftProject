    ...
    const result = (parent) => {
        for (let item of parent.childNodes) {
            if (item.nodeType === Node.ELEMENT_NODE) {
                ...
                if (statistic.tags.hasOwnProperty(tagName)) {
                    statistic.tags[tagName]++;
                } else {
                    statistic.tags[tagName] = 1;
                }

                if (classList.length) {
                    classList.forEach(className => {
                        if (statistic.classes.hasOwnProperty(className)) {
